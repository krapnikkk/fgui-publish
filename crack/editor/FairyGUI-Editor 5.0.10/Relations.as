package fairygui
{
   public class Relations
   {
      ...
      public function add(param1:GObject, param2:int, param3:Boolean = false) : void
      {
         ...
         var _loc5_:RelationItem = new RelationItem(_owner);
      ...
      private function addItems(param1:GObject, param2:String) : void
      {
         ...
         while(_loc7_ < 2)
         {
            ...
            if(_loc4_)
            {
               ...
               _loc9_ = RELATION_NAMES.indexOf(_loc4_);
               if(_loc9_ == -1)
               {
                  throw new Error("invalid relation type");
               }
               _loc5_.internalAdd(_loc9_,_loc6_);
            }
            _loc7_++;
         }
         _items.push(_loc5_);
      ...
      public function onOwnerSizeChanged(param1:Number, param2:Number, param3:Boolean) : void
      {
         ...
         for each(var _loc4_ in _items)
         {
            _loc4_.applyOnSelfResized(param1,param2,param3);
         }
      }
      ...
      public function setup(param1:XML) : void
      {
         ...
         for each(var _loc4_ in _loc2_)
         {
            _loc3_ = _loc4_.@target;
            if(_owner.parent)
            {
               if(_loc3_)
               {
                  _loc5_ = _owner.parent.getChildById(_loc3_);
               }
               else
               {
                  _loc5_ = _owner.parent;
               }
            }
            else
            {
               _loc5_ = GComponent(_owner).getChildById(_loc3_);
            }
            if(_loc5_)
            {
               addItems(_loc5_,_loc4_.@sidePair);
            }
         }
      }
   }
