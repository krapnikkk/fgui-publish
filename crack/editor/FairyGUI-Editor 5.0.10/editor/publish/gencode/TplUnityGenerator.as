package fairygui.editor.publish.gencode
{
   ...
   public class TplUnityGenerator extends GenerateCodeBase
   {
       
      
      public function TplUnityGenerator()
      {
         super();
      }
      ...
      override public function run() : void
      {
         ...
         var _loc2_:String = loadTemplate(publishData.project.type,"Component");
         var _loc3_:Array = [];
         for each(classInfo in sortedClasses)
         {
            ...
            for each(_loc10_ in classInfo.members)
            {
               ...
               if(!_loc10_.ignored)
               {
                  _loc6_.push("\t\tpublic " + _loc10_.type + " " + _loc10_.name + ";");
               }
            }
            _loc3_.push("\t\t\tUIObjectFactory.SetPackageItemExtension(" + classInfo.encodedClassName + ".URL, typeof(" + classInfo.encodedClassName + "));");
            ...
            UtilsFile.saveString(new File(thisPackageFolder.nativePath + File.separator + classInfo.encodedClassName + ".cs"),FILE_MARK + "\n\n" + generate(_loc2_,_loc1_));
         }
         _loc4_ = thisPackageName + "Binder";
         _loc1_["className"] = _loc4_;
         _loc1_["bindContent"] = _loc3_.join("\n");
         UtilsFile.saveString(new File(thisPackageFolder.nativePath + File.separator + _loc4_ + ".cs"),FILE_MARK + "\n\n" + generate(loadTemplate("Unity","Binder"),_loc1_));
