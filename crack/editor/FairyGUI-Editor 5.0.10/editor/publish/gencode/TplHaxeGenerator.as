package fairygui.editor.publish.gencode
{
   ...
   public class TplHaxeGenerator extends GenerateCodeBase
   {
       
      
      public function TplHaxeGenerator()
      {
         super();
      }
      ...
      override public function run() : void
      {
         ...
         init(["hx"]);
         var _loc1_:Object = {};
         _loc1_["packageName"] = thisPackagePath.toLowerCase();
         _loc1_["thisPackageName"] = thisPackageName;
         _loc1_["uiPkgName"] = publishData.pkg.name;
         var _loc2_:String = loadTemplate("Haxe","Component");
         var _loc3_:Array = [];
         for each(classInfo in sortedClasses)
         {
            ...
            for each(_loc10_ in classInfo.members)
            {
               ...
               else
               {
                  if(!_loc10_.ignored)
                  {
                     if(settings.getMemberByName)
                     {
                        _loc5_.push("\t\t" + _loc10_.name + " = cast(this.getChild(\"" + _loc10_.originalName + "\"), " + _loc10_.type + ");");
                     }
                     else
                     {
                        _loc5_.push("\t\t" + _loc10_.name + " = cast(this.getChildAt(" + _loc7_ + "), " + _loc10_.type + ");");
                     }
                  }
                  _loc7_++;
               }
            ...
            _loc3_.push("\t\tUIObjectFactory.setPackageItemExtension(" + classInfo.encodedClassName + ".URL, cast " + classInfo.encodedClassName + ");");
            ...
            _loc1_["createInstance"] = "\t\treturn cast(UIPackage.createObject(\"" + publishData.pkg.name + "\",\"" + classInfo.className + "\"), " + classInfo.encodedClassName + ");";
            _loc1_["variable"] = _loc6_.join("\n");
            _loc1_["content"] = _loc5_.join("\n");
            UtilsFile.saveString(new File(thisPackageFolder.nativePath + File.separator + classInfo.encodedClassName + ".hx"),FILE_MARK + "\n\n" + generate(_loc2_,_loc1_));
         }
         _loc4_ = thisPackageName + "Binder";
         _loc1_["className"] = _loc4_;
         _loc1_["bindContent"] = _loc3_.join("\n");
         UtilsFile.saveString(new File(thisPackageFolder.nativePath + File.separator + _loc4_ + ".hx"),FILE_MARK + "\n\n" + generate(loadTemplate("Haxe","Binder"),_loc1_));
