package fairygui.editor.publish.gencode
{
   ...
   public class TplLayaTSGenerator extends GenerateCodeBase
   {
       
      
      public function TplLayaTSGenerator()
      {
         super();
      }
      ...
      override public function run() : void
      {
         ...
         var _loc2_:String = loadTemplate("LayaTS","Component");
         var _loc3_:Array = [];
            ...
            for each(_loc10_ in classInfo.members)
            {
               ...
               else
               {
                  if(!_loc10_.ignored)
                  {
                     if(settings.getMemberByName)
                     {
                        _loc5_.push("\t\t\tthis." + _loc10_.name + " = <" + _loc11_ + "><any>(this.getChild(\"" + _loc10_.originalName + "\"));");
                     }
                     else
                     {
                        _loc5_.push("\t\t\tthis." + _loc10_.name + " = <" + _loc11_ + "><any>(this.getChildAt(" + _loc7_ + "));");
                     }
                  }
                  _loc7_++;
               }
