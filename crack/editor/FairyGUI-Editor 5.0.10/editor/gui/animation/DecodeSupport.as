package fairygui.editor.gui.animation
{
   ...
   public class DecodeSupport
   {
      ...
      {
         return this._queue.length || this._loadingCount || this._decodingAni != null;
      }
      ...
      private function onTimer() : void
      {
         ...
         var _loc1_:int = 0;
         while(_loc1_ < textureToLoadOneRound)
         {
            ...
            _loc2_ = this._decodingAni.textureList[this._decodingTexture];
            if(!_loc2_.bitmapData)
            {
               if(_loc2_.raw)
               {
                  TextureLoader.load(this._decodingAni,_loc2_,_loc2_.raw,this.onTextureLoaded);
                  this._loadingCount++;
                  _loc1_++;
               }
               else
