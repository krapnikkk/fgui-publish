package fairygui.editor.loader
{
   ...
   public class Downloader extends EventDispatcher
   {
      ...
      public function download(param1:String, param2:String, param3:int = 3, param4:Boolean = true) : void
      {
         ...
         this._sizeLoader.addEventListener("complete",this.onGetSizeComplete);
         this._sizeLoader.addEventListener("ioError",this.onGetSizeError);
      ...
      {
         var _loc3_:* = this._file.nativePath + ".tmp";
         this._tmpFile = new File(_loc3_);
         ...
         this._partCount = Math.ceil((this._fileSize - this._startPosBase) / 1024000);
         this._fileStream = new FileStream();
         this._fileStream.open(this._tmpFile,!!this._allowResuming?"append":"write");
      ...
      private function loadCompleted() : void
      {
         ...
         {
            throw new IOErrorEvent("ioError",false,false,err.message);
         }
      ...
      private function loadPart(param1:PartLoader) : Boolean
      {
         ...
         this._endPos = this._startPos + 1024000 - 1;
         ...
         if(!param1)
         {
            ...
            param1.addEventListener("progress",this.onGetPartProgress);
            param1.addEventListener("complete",this.onGetPartComplete);
            param1.addEventListener("ioError",this.onGetPartError);
      ...
      private function speedHandler() : void
      {
         ...
         var _loc1_:int = _loc2_ - this._lastCheckTime;
         if(_loc1_ < this._checkInterval)
         {
            return;
         }
         ...
         this._lastCheckTime = _loc2_;
         this._speed = this._addedBytes / _loc1_ * 1000;
         this._addedBytes = 0;
      }
      
      private function onGetPartProgress(param1:ProgressEvent) : void
      {
         var _loc3_:int = this._confirmBytes;
         var _loc2_:int = 0;
         while(_loc2_ < this._bodyLoaders.length)
         {
            _loc3_ = _loc3_ + this._bodyLoaders[_loc2_].bytesLoaded;
            _loc2_++;
         }
         param1.bytesLoaded = _loc3_;
         param1.bytesTotal = this._fileSize;
         _loc3_ = param1.bytesLoaded - this._prevBytes;
         if(_loc3_ > 0)
         {
            this._addedBytes = this._addedBytes + _loc3_;
         }
      ...
      {
         var _loc3_:PartLoader = PartLoader(param1.currentTarget);
         this._confirmBytes = this._confirmBytes + _loc3_.bytesLoaded;
         var _loc2_:int = (_loc3_.startPos - this._startPosBase) / 1024000 - this._curWritePart;
         this._writeQueue[_loc2_] = _loc3_.data;
         if(_loc2_ == 0)
         {
            this.doWrite();
         }
      ...
      private function loadNext() : void
      {
         ...
         {
            _loc2_ = this._bodyLoaders[_loc3_];
            if(_loc2_.idle)
            {
               if(this._failedParts.length)
               {
                  _loc1_ = this._failedParts.pop();
                  _loc2_.load(this._url,_loc1_[0],_loc1_[1]);
               }
               else if(this._writeQueue.length < 8)
               {
      ...
      private function onGetPartError(param1:IOErrorEvent) : void
      {
         ...
         {
            _loc2_ = this._bodyLoaders.indexOf(_loc3_);
            this._bodyLoaders.splice(_loc2_,1);
            this._failedParts.push([_loc3_.startPos,_loc3_.endPos]);
            this.loadNext();
         }
      }
